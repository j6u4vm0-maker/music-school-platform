...
interface InventoryTransactionModalProps {
  ...
  modalType: 'STOCK_IN' | 'SALES' | 'SALES_RETURN' | 'PURCHASE_RETURN' | null;
  txQty: number;
...
export default function InventoryTransactionModal({
  ...
  const isReturn = modalType === 'SALES_RETURN';
  const isStockIn = modalType === 'STOCK_IN';
  const isSales = modalType === 'SALES';
  const isSupplierReturn = modalType === 'PURCHASE_RETURN';

  // Stock decreases for Sales and Supplier Returns
  const stockDecreases = isSales || isSupplierReturn;

  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#f8f7f2] w-full max-md rounded-[40px] p-8 md:p-12 shadow-2xl relative border-2 border-[#ece4d9]" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-6 text-center">
          <span className={`inline-block px-4 py-1 rounded-full text-[10px] font-black tracking-[0.2em] mb-4 
            ${isStockIn ? 'bg-[#4a4238] text-white' : isSales ? 'bg-[#c4a484] text-white' : isReturn ? 'bg-purple-600 text-white' : 'bg-blue-600 text-white'}`}>
            {isStockIn ? 'IN STOCK / 進貨入庫' : isSales ? 'SALES / 結帳售出' : isReturn ? 'RETURN / 銷售退貨' : 'RETURN TO SUPPLIER / 退貨給供應商'}
          </span>
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
             ...
             <input 
               type="number" 
               min="1" 
               max={stockDecreases ? selectedProduct.stockQty : undefined}
               required
               value={txQty}
             ...
             />
             {stockDecreases && txQty > selectedProduct.stockQty && (
               <p className="text-red-500 text-[10px] font-bold ml-2">警告：操作數量大於現有庫存！</p>
             )}
          </div>
          <div className="flex flex-col gap-2">
             <label className="text-xs font-black text-[#4a4238] tracking-widest ml-2">
               {isStockIn || isSupplierReturn ? '單位進價 (Cost Price)' : '單位售價 (Sell Price)'}
             </label>
          ...
          <div className="mt-4 bg-[#ece4d9]/30 p-4 rounded-2xl border border-[#ece4d9] text-center flex flex-col gap-1">
             <p className="text-[10px] font-bold tracking-widest text-[#4a4238]/60 uppercase">
                {isStockIn ? 'Estimated Expense (總支出)' : (isSales ? 'Estimated Revenue (總收入)' : (isReturn ? 'Refund Total (退款總額)' : 'Supplier Refund (供應商退款)'))}
             </p>
             <p className={`font-mono text-3xl font-black ${isReturn ? 'text-purple-600' : isSupplierReturn ? 'text-blue-600' : 'text-[#4a4238]'}`}>
                {(isReturn || isSupplierReturn) ? '-' : ''}${ (txQty * txPrice).toLocaleString() }
             </p>
          </div>

          <button 
            type="submit" 
            disabled={isSubmitting || (stockDecreases && txQty > selectedProduct.stockQty)}
            className={`w-full mt-4 py-4 rounded-full font-black tracking-[0.2em] text-sm transition-all shadow-md
              ...
              isStockIn ? 'bg-[#4a4238] hover:bg-[#322c26] text-white' : 
              isSales ? 'bg-[#c4a484] hover:bg-[#b08d6d] text-white' :
              isReturn ? 'bg-purple-600 hover:bg-purple-700 text-white' :
              'bg-blue-600 hover:bg-blue-700 text-white'}
              hover:shadow-xl hover:-translate-y-1 disabled:opacity-50 disabled:hover:translate-y-0
            `}
          >
